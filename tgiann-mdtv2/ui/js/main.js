    ...
    $.post('http://tgiann-mdtv2/sorgula', JSON.stringify({data: data, tip: tip}), function(cbData) {
        ...
        if (playerData.length) {
            ...
            for (let index = 0; index < playerData.length; index++) {
                ...
                let name = JSON.parse(element.charinfo).firstname + " " + JSON.parse(element.charinfo).lastname
                if (tip == "plaka") { 
                    name = name + " (" + element.plate + ")" 
                } else if (tip == "numara") { 
                    name = name + " (" + JSON.parse(element.charinfo).phone + ")" 
                }
                htmlSorgu = htmlSorgu + `
...
$(document).on("click",".kullanici",function() {
    ...
    zanliIsim = JSON.parse(userData.charinfo).firstname + " " + JSON.parse(userData.charinfo).lastname
    zanliCid = userData.citizenid
    ...
    $("#kullaniciBilgi").html(`
        <div class="sorgu-sag-alt-sol">
            ...
            <div class="bilgi"><div class="bilgi-sol">İsim</div><div class="bilgi-sag">${JSON.parse(userData.charinfo).firstname}</div></div>
            <div class="bilgi"><div class="bilgi-sol">Soyisim</div><div class="bilgi-sag">${JSON.parse(userData.charinfo).lastname}</div></div>
            <div class="bilgi"><div class="bilgi-sol">Vatandaş No</div><div class="bilgi-sag">${JSON.parse(userData.charinfo).account}</div></div>
            <div class="bilgi"><div class="bilgi-sol">Kimlik Numarası</div><div class="bilgi-sag">${userData.citizenid}</div></div>
        </div>

        <div class="sorgu-sag-alt-sag">
            <div class="bilgi"><div class="bilgi-sol">Telefon Numarası</div><div class="bilgi-sag">${JSON.parse(userData.charinfo).phone}</div></div>
            <div class="bilgi"><div class="bilgi-sol">Banka</div><div class="bilgi-sag">${Intl.NumberFormat('en-US').format(JSON.parse(userData.money).bank)}$</div></div>
            <div class="bilgi"><div class="bilgi-sol">Uyruk</div><div class="bilgi-sag">${JSON.parse(userData.charinfo).nationality}</div></div>
            <div class="bilgi"><div class="bilgi-sol">Doğum Tarihi</div><div class="bilgi-sag">${JSON.parse(userData.charinfo).birthdate}</div></div>
            <div class="bilgi"><div class="bilgi-sol">Ev</div><div class="bilgi-sag" id="ev">Yükleniyor...</div></div>
        </div>
    ...
    $.post('http://tgiann-mdtv2/ev', JSON.stringify({id: secilenCitizenid}), function(cbData) {
        ...
        if (cbData == "Ev Bilgisi Yok!") {
            ...
            raidButton = `
                ...
                <button class="sorgu-button-2-mini" id="ev-r" data-ev="${cbData}">Ev Raid</button>
            `
            $("#ev").html(cbData)
        }
